...
export default function VerifyPickupOTPScreen() {
  ...
  const handleRequestOTP = async () => {
    try {
      setRequesting(true);
      const response = await bookingService.requestPickupOTP(bookingId);

      setMaskedPhone(response.customer_phone || 'customer');
      setOtpRequested(true);

      // Stay on page - show info message
      setInfoMessage(response.message || t.verifyOtp.otpSentToCustomer);
      setShowInfo(true);
    } catch (error: any) {
      setErrorMessage(error.message || 'Failed to send OTP. Please try again.');
      setShowError(true);
      // On failure, route to home page after showing error
      setTimeout(() => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Vendor cannot access owner's OTP endpoint, so we don't try to send notification here

      // Show success - will route to home on modal close
      setShowSuccess(true);
    } catch (error: any) {
      setErrorMessage(error.message || 'Invalid OTP. Please check and try again.');
      setShowError(true);
      // On verification failure, route to home page after showing error
      setTimeout(() => {
        router.replace('/(tabs)/home');
      }, 2000);
    } finally {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background.secondary }]}>
      ...
      {/* Header */}
      <ScreenHeader title={t.verifyOtp.confirmPickupTitle} />
          ...
          <View style={[styles.content, { backgroundColor: theme.colors.background.secondary }]}>
            ...
            <View style={[styles.infoCard, { backgroundColor: theme.colors.background.primary }]}>
              ...
              <Text style={[styles.infoText, { color: theme.colors.text.secondary }]}>
                {t.verifyOtp.requestOtpPickupDescription}
              </Text>
              ...
              <>
                ...
                >
                  {verifying ? (
                    ...
                    <Text style={styles.verifyButtonText}>{t.verifyOtp.verifyAndConfirmPickup}</Text>
                  )}
                </TouchableOpacity>
              </>
            )}
      ...
      <SuccessModal
        ...
        onClose={() => {
          ...
          // Navigate to home instead of order-detail to avoid back button issues
          router.replace('/(tabs)/home');
        }}
        title={t.verifyOtp.pickupConfirmedTitle}
        message={t.verifyOtp.pickupConfirmedMessage}
        icon="success"
